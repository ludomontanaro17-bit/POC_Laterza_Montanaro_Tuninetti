...
# Carica dataset
csv_path = os.path.join(path, "loan_sanction_train.csv")
...
target_counts = prep.y.value_counts().rename_axis('Loan_Status').reset_index(name='count')
...
for col in cats:
    if col in df.columns:
        ...
        csv_name = f"{col.lower()}_counts.csv"
        ct.to_csv(os.path.join(out_dir, csv_name), index=False)
        cat_summaries[col] = ct

# 3) Statistiche numeriche per Income e LoanAmount
num_stats = df[['ApplicantIncome','CoapplicantIncome','LoanAmount']].agg(['mean','median','std','min','max']).T
num_stats = num_stats.round(2).reset_index().rename(columns={'index':'feature'})
...
if 'Credit_History' in df.columns:
    ...
    pct_pos = (ch_valid == 1).sum() / len(ch_valid) * 100 if len(ch_valid)>0 else None
    with open(os.path.join(out_dir, "credit_history_summary.txt"), "w") as f:
        f.write(f"Percentuale Credit_History positivo (su non-mancanti): {pct_pos:.2f}%")

# 5) Correlazioni numeriche e heatmap
num_df = df.select_dtypes(include=['int64','float64']).copy()
# temporaneamente mappa target a 0/1 per correlazioni
if 'Loan_Status' in num_df.columns:
    pass
else:
...
# 7) Salvataggio tabelle ed eventuale messaggio
